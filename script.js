...
public class PartTwoDriver implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        // Clean existing output paths to avoid errors
        if (fs.exists(intermediatePathTF)) {
            fs.delete(intermediatePathTF, true);
        }
        if (fs.exists(intermediatePathIDF)) {
            fs.delete(intermediatePathIDF, true);
        }
        if (fs.exists(outputPathTFIDF)) {
            fs.delete(outputPathTFIDF, true);
        }
        ...
        if (!tfJob.waitForCompletion(true)) {
            return 1;  // If TF Job fails, stop execution
        }
        ...
        if (!idfJob.waitForCompletion(true)) {
            return 1;  // If IDF Job fails, stop execution
        }
        ...
        if (!tfidfJob.waitForCompletion(true)) {
            return 1;  // If TF-IDF Job fails, stop execution
        }
